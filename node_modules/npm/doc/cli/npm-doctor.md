...
+ The directories that npm uses, `node_modules` (both locally and globally),
  exist and can be written by the current member.
...
* Your cache must be readable and writable by the member running npm.
* Global package binaries must be writable by the member running npm.
* Your local `node_modules` path, if you're running `npm doctor` with a project
  directory, must be readable and writable by the member running npm.
