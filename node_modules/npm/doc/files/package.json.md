...
npm also sets a top-level "maintainers" field with your npm member info.
...
That is, if your package is named `foo`, and a member installs it, and then does
...
`npm_package_config_port` environment variable, then the member could
...
* `member/repo` See 'GitHub URLs' below
...
Git urls are of the form:
    ...
    <protocol>://[<member>[:<password>]@]<hostname>[:<port>][:][/]<path>[#<commit-ish> | #semver:<semver>]
...
"member/foo-project".  Just as with git URLs, a `commit-ish` suffix can be
...
Unless the member has set the `engine-strict` config flag, this
...
member had set `engine-strict`. It is no longer used.
