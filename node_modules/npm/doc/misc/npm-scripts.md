...
to the member account or uid specified by the `member` config, which
...
* Inspect the env to determine where to put things.  For instance, if
  the `npm_config_binroot` environment variable is set to `/home/member/bin`, then
  don't try to install executables into `/usr/local/bin`.  The member
