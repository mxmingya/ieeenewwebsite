...
to the member account or uid specified by the \fBuser\fP config, which
...
.RE
.P
then the member could change the behavior by doing:
.P
.RS 2
.nf
...
the \fBnpm_config_binroot\fP environment variable is set to \fB/home/member/bin\fP, then
don't try to install executables into \fB/usr/local/bin\fP\|\.  The member
probably set it up that way for a reason\.
.IP \(bu 2
Don't prefix your script commands with "sudo"\.  If root permissions
are required for some reason, then it'll fail with that error, and
the member will sudo the npm command in question\.
.IP \(bu 2
