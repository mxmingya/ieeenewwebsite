...
* [authentication](https://github.com/npm/registry/blob/master/docs/member/authentication.md)
* [profile editing](https://github.com/npm/registry/blob/master/docs/member/profile.md) (and two-factor authentication)

## Functions

### profile.adduser(opener, prompter, config) → Promise

Tries to create a member new web based login, if that fails it falls back to
using the legacy CouchDB APIs.

* `opener` Function (url) → Promise, returns a promise that resolves after a browser has been opened for the member at `url`.
...
* `username` String, the username the member authenticated as
...
Creates a new member on the server along with a fresh bearer token for future
authentication as this member.  This is what you see as an `authToken` in an
`.npmrc`.

If the member already exists then the npm registry will return an error, but
...
Logs you into an existing member.  Does not create the member if they do not
...
Fetch profile information for the authenticated member.
...
Update profile information for the authenticated member.
...
and `new` properties, where the former has the member's current password and
...
2. `profile.set({tfa: {password, mode}}, {registry, auth: {token}})`
   * Note that the member's `password` is required here in the `tfa` object,
...
3. The response will have a `tfa` property set to an `otpauth` URL, as
   ...
   You will need to show this to the member for them to add to their
...
4. To complete setting up two factor auth you need to make a second call to
   `profile.set` with `tfa` set to an array of TWO codes from the member's
...
Fetch a list of all of the authentication tokens the authenticated member has.
...
* token — The first six characters of the token UUID.  This should be used
  by the member to identify which token this is.
