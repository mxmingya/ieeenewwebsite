...
Add a member account to the registry, or verify the credentials.
...
Note that the member does not have to be the package owner to star or unstar a
package, though other writes do require that the member be the package owner.

### client.stars(uri, params, cb)

* `uri` {String} The base URL for the registry.
* `params` {Object} Object containing per-request properties.
  * `username` {String} Name of member to fetch starred packages for. Optional
    (default: member in `auth`).
  * `auth` {Credentials} Optional (required if `username` is omitted).
* `cb` {Function}

View your own or another member's starred packages.
