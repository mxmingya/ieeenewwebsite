...
`member:password` before the host with an `@` sign:
...
request.post({url:url, oauth:oauth}, function (e, r, body) {
  ...
  // and construct a URL that a member clicks on (like a sign in button).
  // The verifier is only available in the response after a member has
  ...
  // redirect the member to the authorize uri

  // step 3
  // after the member is redirected back to your server
  var auth_data = qs.parse(body)
    ...
    ;
  request.post({url:url, oauth:oauth}, function (e, r, body) {
    // ready to make signed requests on behalf of the member
    var perm_data = qs.parse(body)
      ...
      ;
    request.get({url:url, oauth:oauth, qs:qs, json:true}, function (e, r, member) {
      console.log(member)
    })
  })
})
...
User-Agent: whatever member agent you specify
```
...
- `auth` - a hash containing values `member` || `username`, `pass` || `password`, and `sendImmediately` (optional). See documentation above.
