...
<h2 id="member">USER</h2>
<p>If npm was invoked with root privileges, then it will change the uid
to the member account or uid specified by the <code>member</code> config, which
...
</code></pre><p>then the member could change the behavior by doing:</p>
...
the <code>npm_config_binroot</code> environment variable is set to <code>/home/member/bin</code>, then
don&#39;t try to install executables into <code>/usr/local/bin</code>.  The member
...
the member will sudo the npm command in question.</li>
