...
<li>per-member configuration file (defaults to <code>$HOME/.npmrc</code>; configurable via CLI
...
should be polled while the member is completing authentication.</p>
...
scripts.  If set explicitly to false, then installing as a non-root member
will fail.</p>
...
<p>The location of member-level configuration settings.</p>
...
<h3 id="member-agent">member-agent</h3>
<ul>
